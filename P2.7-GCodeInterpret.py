...
def read_gCode():
    ...
    fileName = raw_input("File name (must be in current directory): ") # Change made
...
def execute_command(commandList):
    global prevTool, totalRotation, ToolLoc, drawHeight, moveHeight, toolHeight # Used to remove "UnboundLocalError: local variable ___ referenced before assignment"
    ...
    for command in commandList:
        ...
        totalRotation = totalRotation + a # what is "a"?
        
        

def set_step_params(x, y, a, h):
    global prev_11, prev_12, prev_13, x1, x2, x3, y1, y2, y3

    # If we want to use math library, put "math.____"
    ...
    eng.udp_setup(angle_to_steps(a), 1, fsign(a), len_to_steps(l1-prev_l1), 1, fsign(l1-prev_l1), len_to_steps(l2-prev_l2), 1, fsign(l2-prev_l2), len_to_steps(l3-prev_l3), 1, fsign(l3-prev_l3), 0)
